...
void MapIntersectionJob::intersection_loop()
{
    ...
    {
        timestep.mark_start();
        if (!Game::instance().intersection.requested.load())
        {
            timestep.mark_end();
            timestep.delay();
        }
        ...
        glm::vec3 direction;
        auto& map = Game::instance().get_map();
        ...
        float closest_distance = std::numeric_limits<float>().max();
        {
            ...
            for (uint32_t tile_x = 0; tile_x < Map::CHUNK_DIMENSION; tile_x++) {
                for (uint32_t tile_y = 0; tile_y < Map::CHUNK_DIMENSION; tile_y++) {
                    ...
                    if (!map.tiles[tile_x][tile_y].clickable) continue;
        ...
        if (Game::instance().intersection.requested.load())
        {
            if (best_result_tile_x >= 0)
            {
                ...
                Game::instance().intersection.found.store(true);
                Game::instance().intersection.x_tiles.store(best_result_tile_x);
                Game::instance().intersection.y_tiles.store(best_result_tile_y);

                auto& highlight = Resources::instance().render_entities.selected_tile_highlight;
                highlight.position = { best_result_tile_x, 0.1f, best_result_tile_y };
                blue::Context::renderer().update(highlight);

                // Place unit, just for tests.
                //auto currentOccupant = Game::instance().get_map().get_tile_occupant(best_result_tile_x, best_result_tile_y);
                //if (currentOccupant == UnitType::NONE)
                //{
                //	Game::instance().get_map().set_tile_occupant(UnitType::TANK, best_result_tile_x, best_result_tile_y);
                //}
                //else
                //{
                //	// Highlight
                ...
                Game::instance().intersection.x.store(intersection_point.x);
                Game::instance().intersection.y.store(intersection_point.z);
            }
            else
            {
                auto& highlight = Resources::instance().render_entities.selected_tile_highlight;
                highlight.position = { 100.0f, 100.0f , 100.0f };
                blue::Context::renderer().update(highlight);
            }
        }

        Game::instance().intersection.requested.store(false);
